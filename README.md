
# readme provisório

# analise-laureados-premio-nobel
o projeto tem como objetivo analisar os laureados com o Prêmio Nobel.

## dados desejados
- vencedores por categoria em cada ano: https://pt.wikipedia.org/wiki/Laureados_com_o_Nobel#1930

- ddf laureados: https://www.kaggle.com/datasets/joebeachcapital/nobel-prize/data

- mulheres vencedoras do prêmio nobel: https://pt.wikipedia.org/wiki/Lista_de_mulheres_laureadas_com_o_Nobel

- nivel de democracia dos países: https://ourworldindata.org/grapher/democracy-index-eiu?download-format=tab&utm_source=chatgpt.com


ano
sexo
categoria
idade quando venceu
pais
região global (oriente médio, europa, américa do norte, etc)

- Evolução da participação feminina

- Nobel per capita

- Em que áreas cada país se destaca?

- As guerras afetaram as premiações?

- Quem ganhou mais de uma vez?

- Quais instituições aparecem mais?

- existe correlação entre o número de laudeados com os seguintes dados?:
    - idh do país
    - nível de democracia
    - população
    - pib per capita
    - investimeno em P&D
